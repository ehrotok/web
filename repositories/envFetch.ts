export const $envFetch = async <T>(url: string, opts: any = {}): Promise<T> => {
  ...
  if (config.public.isDevelopment) {
    return !opts.method || opts.method === 'GET' ? fetchDev<T>(url) : Promise.resolve({} as T)
  }
...
const fetchDev = async <T>(url: string): Promise<T> => {
  ...
  if (url === Constants.API_URLS.ACCOUNTS) {
    return JSON.parse(`{
  ...
  return JSON.parse(response.files[config.public.development.fileName].content) as T
}
